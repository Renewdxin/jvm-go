...
func (ob *Object) ArrayLength() int32 {
	switch ob.data.(type) {
	case []int8:
		return int32(len(ob.data.([]int8)))
	case []int16:
		return int32(len(ob.data.([]int16)))
	case []int32:
		return int32(len(ob.data.([]int32)))
	case []int64:
		return int32(len(ob.data.([]int64)))
	case []uint16:
		return int32(len(ob.data.([]uint16)))
	case []float32:
		return int32(len(ob.data.([]float32)))
	case []float64:
		return int32(len(ob.data.([]float64)))
	case []*Object:
		return int32(len(ob.data.([]*Object)))
	default:
		panic("Not array!")
	}
}
